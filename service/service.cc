...
Status ServiceImpl::chirp(ServerContext *context, const ChirpRequest *request,
  ...
  auto user_id =
      client.PutOrUpdate(USER_ID + request->username(), replyC->id());
  if (!user_id.ok())
  ...
  if (client.Has(parent).ok()) {
    ...
    auto id_create = client.PutOrUpdate(ID_REPLY + replyC->id(), "");
    if (!id_create.ok())
      return Status(StatusCode::ALREADY_EXISTS, "Reply error");
  } else {
    auto id_reply = client.PutOrUpdate(ID_REPLY + replyC->id(), "");
    if (!id_reply.ok())
      return Status(StatusCode::ALREADY_EXISTS, "Reply error");
  }
  return Status::OK;
