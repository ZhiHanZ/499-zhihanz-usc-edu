...
class ServiceImpl final : public ServiceLayer::Service {
  public:
    ...
    //Get the relevent key value pair to retrive the most recent chirp id pushed buy given user
    //and return the id string
    auto GetUserId(const string& username);
    //Get all users that the given username followed and return a vector of followed people
    auto GetUserFollowed(const string& username);
    //Get all replied chirp is through the given chirp id and return a vector of replied chirp id
    ...
    //Receive username, chirp text, parent_id in request and store the chirp string and relevent information
    //to key value store
    Status chirp(ServerContext* context, const ChirpRequest* request, ChirpReply* reply)override;
    //Receive the username and to_follow name in user request and send all information to key value stor
