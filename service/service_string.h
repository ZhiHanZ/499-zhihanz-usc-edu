...
class FakeService final {
 public:
  ...
  auto GetNumMonitorLoop(){
    return monitor_refresh_times_;
  }
  auto SetNumMonitorLoop(int64_t loop_num){
    monitor_refresh_times_ = loop_num;
  }
  auto GetRefreshTimeVal(){
    return refresh_timeval_;
  }
  ...
  // 5ms time it will loop over all followed people and send newly published data to chirpReply
  ...
  //using mutex monitor_mutex_ and condition_variable monitor_buf_signal_ to synchronize
  //examle usage:
  //If you want to buffer the information from monitor
  //FakeService service;
 ...
 private:
  IdGenerator id_generator_;  // dependency injection, This object can generate id
